...
export class BoardComponent implements OnInit {
  ...
  boards: any = {
    Development: {
      labels: ['To do', 'TodayManday', 'In progress', 'Local done', 'Tests', 'Changelog'],
      filterLabel: '',
    },
    Frontend: {
      labels: ['To do', 'TodayManday', 'In progress', 'Local done'],
  ...
  getGroupMembers(groupId: any) {
    this.groupsService.getAssigneesByGroup(groupId).subscribe((response: any) => {
      this.assignees = response;
    });
  }

  getGroupLabels(groupId: number) {
    this.groupsService.getLabelsByGroup(groupId).subscribe((response) => {
      const labelsLocal = {};
  ...
  showBoard(board: any) {
    ...
    this.groupsService.getGroupDetailsById(board.id).subscribe((response: any) => {
